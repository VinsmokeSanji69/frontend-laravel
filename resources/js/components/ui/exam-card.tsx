...
import DuplicateExam from "@/pages/form/duplicate-exam"
import DeleteExam from "@/pages/form/delete-exam"
...
export function ExamCard({
    ...
    const handleCardClick = (e: React.MouseEvent<HTMLDivElement>) => {
        // Check if click is on an action button or dropdown
        const target = e.target as HTMLElement
        if (target.closest('[data-action="card-action"]')) {
    ...
    return (
        <>
            <div
                ...
                className={cn(
                    "group relative flex flex-col w-full border-2 border-card-foreground rounded-md gap-1 px-3 py-2 shadow-[6px_6px_0_#000000] transition-all hover:-translate-y-0.5 cursor-pointer",
                    getColor(index),
                    className
                )}
                {...props}
